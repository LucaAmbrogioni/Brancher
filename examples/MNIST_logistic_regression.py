...
#dataset_size = 50
input_variable = np.array([np.reshape(image[0], newshape=(number_pixels, 1)) for image in train][0:dataset_size])
output_labels = np.array([image[1]*np.ones((1, 1)) for image in train][0:dataset_size])
...
test_images = EmpiricalVariable(np.array([np.reshape(image[0], newshape=(number_pixels, 1)) for image in test]),
                                indices=test_indices, name="x_test", is_observed=True)
test_labels = EmpiricalVariable(np.array([image[1]*np.ones((1, 1))
                                          for image in test]), indices=test_indices, name="labels", is_observed=True)
...
for _ in range(num_images):
    ...
    model_output = model._get_posterior_sample(10, input_values={x: test_image})[k].detach().numpy()
    output_probs = np.reshape(np.mean(model_output, axis=0), newshape=(10,))
    s += 1 if int(np.argmax(model_output)) == int(test_label.detach().numpy()) else 0
